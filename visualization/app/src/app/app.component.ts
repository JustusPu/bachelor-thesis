import { Component, OnInit, ViewChild, ViewEncapsulation, AfterViewInit, AfterContentInit } from '@angular/core';
...
export class AppComponent implements OnInit {
    ...
    if (this.step == 1) {
      ...
      this.anchors.forEach(elem => {
        if (elem.pos) {
          ...
          this.tagCOS.addNode({ x: elem.pos.x * 100, y: elem.pos.y * 100, z: elem.pos.z * 100 }, "#0000ff");
        }
      })
      this.cloud.anchors.forEach(elem => {
        if (elem.pos) {
          // console.log(elem.name,elem.pos);
          this.tagCOS.addNode({ x: elem.pos.x * 100 / accuracy, y: elem.pos.y * 100 / accuracy, z: elem.pos.z * 100 / accuracy }, "#00ff00");
        }
      })
      this.step++;
    }
  ...
  loadSetup(target) {
    ...
    target.value = "";
    this.messageService.add({ sticky: true, severity: 'success', summary: 'Setup wurde erfolgreich geladen', detail: '' });
    this.step = 2;
  }
  saveSetup() {
  ...
  onError(event) {
    console.log(event)
    this.messageService.add({ sticky: true, severity: 'error', summary: 'Fehler beim Setup-Laden', detail: 'Für mehr Details bitte die Konsole öffnen' });
  }
  ...
  generateTagTimes(tagsCount, rangeUWB, rangeToleranceUWB, accuracyUWB) {
    ...
    if (this.setupCOS.lat && this.setupCOS.lon && this.setupCOS.sortedLayers.length > 0 && this.setupCOS.length && this.setupCOS.width && this.setupCOS.rotation && tagsCountVal > 0 && rangeUWBVal > 0 && rangeToleranceUWBVal >= 0 && accuracyUWBVal >= 0) {
      // let buildingOrigin = functions.altlatlon2ecef({ lat: this.setupCOS.lat, lon: this.setupCOS.lon, alt: this.setupCOS.alt });
      let tags = [];
      ...
      for (let i = 0; i < tagsCountVal; i++) {
        ...
        // console.log(pos)
        last = pos;
        // let lla = functions.ecef2altlatlon(functions.local2ecef(buildingOrigin, pos));
        let lla = { lat: null, lon: null, alt: null };
        let neighbours = {}
        ...
        // this.tagCOS.addNode({ x: pos.x * 100, y: pos.y * 100, z: pos.z * 100 }, "#00ffff")
        tags.push({ time: Date.now() + i, pos: pos, lla: lla, neighbours: neighbours })
      }
  ...
  addTagTime() {
    this.tags = [...this.tags, { time: Date.now(), pos: { x: null, y: null, z: null }, lla: { lat: null, lon: null, alt: null }, neighbours: {} }];
  }
  removeTagTime() {
  ...
  updateTag() {
    if (this.selectedTag) {
      let anchor = new Anchor("Tag", { x: this.selectedTag.pos.x * 100, y: this.selectedTag.pos.y * 100, z: this.selectedTag.pos.z * 100 });
      for (let neighbour in this.selectedTag.neighbours) {
        anchor.addNeighbour(this.cloud.getNodeByName(neighbour), this.selectedTag.neighbours[neighbour] * 100);
      }
      let pos = this.cloud.getTagPosition(anchor)
      console.log(this.selectedTag.pos, pos);
      this.currentlla = functions.ecef2altlatlon(functions.local2ecef(this.buildingOrigin, { x: pos.x / 100, y: pos.y / 100, z: pos.z / 100 }));
      this.tagCOS.setTagPosition( { x: pos.x / 100, y: pos.y / 100, z: pos.z / 100 });
      if (this.showTrack) { this.tagCOS.addNode({ x: this.selectedTag.pos.x * 100, y: this.selectedTag.pos.y * 100, z: this.selectedTag.pos.z * 100 }, "#00ffff") }
    }
  }
}